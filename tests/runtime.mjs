...
export async function bundle() {
  const result = await build({
    entryPoints: [new URL('../UAKino/src/index.js', import.meta.url).pathname],
    bundle: true, format: 'iife', globalName: 'PluginModule', platform: 'neutral',
    target: 'es2020', write: false, footer: { js: 'Object.assign(globalThis, PluginModule);' },
  });
  return result.outputFiles[0].text;
}
export function runtime(code, request, baseUrl = manifest.baseUrl) {
  const calls = [];
  ...
  const context = vm.createContext({
    manifest: { ...manifest, baseUrl }, URL, console,
