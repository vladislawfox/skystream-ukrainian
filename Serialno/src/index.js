...
const SEARCH_PATH = '/';
const sections = [['Серіали', '/series/page/', 'series'], ['Мультсеріали', '/cartoons/page/', 'series'], ['Міні-серіали', '/mini-serials/page/', 'series']];
async function cards(html) {
  ...
  for (const node of await select(html, '.th-item')) {
    ...
    if (url && title) items.push({ title, url, posterUrl: await poster(body, '.img-fit img'), type: 'series', headers: headers() });
  }
...
async function detail(url) {
  ...
  if (!title) throw new ProviderError('PARSE_ERROR', 'Не знайдено назви Serialno. Структура сайту могла змінитися.');
  const info = await select(html, '.flist li');
  const trailer = await tabTrailer(html, '.tabs-sel span', '.video-box', url);
  const players = await playersFrom(html, 'div.video-box iframe', url, trailer);
  const item = { title, url, posterUrl: absolute(await attribute(html, '.fposter a', 'href'), url) || await poster(html, '.fposter img', url), description: await text(html, '.full-text'), tags: await values(inner(info[info.length > 4 ? 4 : 3]), 'a'), cast: [], trailers: [], recommendations: [], headers: headers(), type: 'series' };
  addYearAndScore(item, await text(inner(info[1]), 'a'), await text(html, '.th-voice'));
